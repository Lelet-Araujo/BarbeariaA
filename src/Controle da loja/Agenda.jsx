...
export default function AgendaPage() {
  ...
  const funcionarioAgenda = searchParams.get("funcionario");
  const barbeirosBase = ["Carlos", "João", "Artur"];
  const barbeiroDaUrl = barbeirosBase.find(
    (barbeiro) => funcionarioAgenda === barbeiro || funcionarioAgenda?.startsWith(`${barbeiro} `)
  ) || funcionarioAgenda;
  const barbeiros = barbeiroDaUrl && !barbeirosBase.includes(barbeiroDaUrl)
    ? [...barbeirosBase, barbeiroDaUrl]
    : barbeirosBase;

  const [barbeiroSelecionado, setBarbeiroSelecionado] = useState(
    barbeiroDaUrl || barbeiros[0]
  );
